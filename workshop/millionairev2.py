...
def measure(a, b):
    ...
    n, result = len(a), 0
    for i in range(0,n):
        numer = (a[i] - meanOfA ) * (b[i] - meanOfB)
        denom = float(stdOfA * stdOfB * n)
        result += (numer/denom)
    return result

def computeDelta(X, S):
    """
    ...
    for i in range(len(matrx)):
        exp = math.exp(2.7182818 * measure(X[:n], matrx[i][:n]))
        exp = math.exp(2.7182818 * measure(X[:n], matrx[i][:n]))
        numerator +=  (matrx[i][n] * exp)
        denominator += math.exp(wt * measure(X[:n], matrx[i][:n]))
